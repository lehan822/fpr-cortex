...
> AgentCore agents skip this — runtime handles auth via IAM/SigV4.

## Daemon model — start once, forget

A background daemon keeps tokens fresh. The agent bootstraps it once, then never worries about auth again.

**Agent auto-bootstrap (on first use):**
1. Write the Python script below to `~/.fpr/fpr-auth.py`
2. `python3 ~/.fpr/fpr-auth.py prod` → opens browser for first login
3. `python3 ~/.fpr/fpr-auth.py daemon prod` → starts daemon (refreshes silently in background)

After that, `python3 ~/.fpr/fpr-auth.py prod` always returns instantly — daemon keeps the token fresh.

**Commands:**
| Command | Purpose |
|---------|---------|
| `python3 ~/.fpr/fpr-auth.py <env>` | Check/refresh/login (daemon makes this instant) |
...
Local agent uses two tokens from `~/.fpr/auth.json`:

| Token | Goes into |
|-------|-----------|
| id_token | request `context.authServiceToken` |
| access_token | `Authorization: Bearer` header |

AgentCore: user id_token from caller; M2M auto-managed by IAM.
